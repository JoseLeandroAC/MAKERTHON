...
def reconhecer_rosto_api():
    try:
        ...
        if not resultados_df or resultados_df[0].empty:
            ...
            print("DataFrame retornado pelo DeepFace:")
            print(resultados_df[0].to_markdown(index=False))
        print("-------------------------\n")

        if resultados_df and not resultados_df[0].empty:
            # Ordena por distância para pegar a correspondência mais próxima
            df = resultados_df[0].sort_values(by="distance", ascending=True)
            distancia = df['distance'].iloc[0]
            if distancia <= LIMITE_CONFIANCA:
                caminho_identidade = df['identity'].iloc[0]
